...
public class _SkillProjectile : _SkillBase
{
    ...
    public void InitSkillObject2D(int ownerInstanceId, bool isDirRight, Vector3 playerPos, int damage, string parentTag, Action<int, int> onSkillCollision = null) 
    {
        ...
        InitSkillObject(ownerInstanceId, damage, parentTag, onSkillCollision);
    }

    public void InitSkillObject3D(int ownerInstanceId, Vector3 playerPos, Vector3 playerDir, int damage, string parentTag, Action<int, int> onSkillCollision = null)
    {
        this.transform.position = playerPos;

        _moveDirection = playerDir.normalized;
        this.transform.forward = playerDir;

        InitSkillObject(ownerInstanceId, damage, parentTag, onSkillCollision);
    }

    private void InitSkillObject(int ownerInstanceId, int damage, string parentTag, Action<int, int> onSkillCollision = null)
    {
        _damage = damage;
    ...
    private void CheckCollision(Collider2D collision)
    {
        ...
        else if(collision.CompareTag("Enemy") && (isOwnerPlayer))  // 몬스터다! (대신 Tag 오탈자, 대소문자 꼭 확인)
        {
            ...
            var monsterComponent = gObj.GetComponent<BattleAgent>();
